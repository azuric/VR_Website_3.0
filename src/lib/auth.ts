...
export async function checkUsernameAvailability(username: string): Promise<boolean> {
  try {
    ...
    if (error) {
      ...
      // If there's a database error (like table doesn't exist), assume username is available
      // This allows signup to work even if database isn't fully set up
      return true
    }
    ...
    console.error('Check username availability error:', error)
    // On any error, allow the username (better UX than blocking signup)
    return true
  }
}
